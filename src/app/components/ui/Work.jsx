...
export default function Work() {
  ...
  const handleScroll = () => {
   ...
   if (section) {
     ...
     const scrolled = Math.min(Math.max(((window.scrollY - top) - height) / totalHeight, 0), 1);
     console.log('height : '+ height,'top : ' + top,'scrollY : '+ scrollY,'innerHeight : ' + innerHeight)
  ...
  return (
    <section>
      ...
      <div id='scroll-section' className={`grid grid-cols-[1%_50%,49%]
      bg-[url('/Frame1.png')] bg-cover bg-no-repeat bg-center lg:my-20 md:my-14 my-10 relative  mx-4 lg:mx-10 `} >
        <div className='bg-[#1F2B15] max-w-[4px]  h-[100%] translate-y-[10%] ' >
         <span className=' bg-[#0ae106] block' style={{height: `${progress}%`}}>
        ...
        <div className=' mx-2 sm:mx-4 md:mx-6 lg:mx-8 flex flex-col lg:gap-10 sm:gap-6 gap-4 mt-4 lg:mt-8'>
          {workPoints.map((working) => {
            return (
              <div key={working.id} id={working.id} className={`lg:pr-10 md:pr-6 sm:pr-3 flex flex-col sm:gap-3 gap-1 ${activeSection===working.id?'opacity-100':'opacity-50'}`}>
